...
(function (doc, win) {
  ...
  donutty = win.Donutty = function (el, options) {
    ...
    this.options.min = isDefined(this.options.min) ? _float(this.options.min) : 0;
    this.options.max = isDefined(this.options.max) ? _float(this.options.max) : 100;
    this.options.value = isDefined(this.options.value) ? _float(this.options.value) : 50;
    ...
    this.options.padding = isDefined(this.options.padding) ? _float(this.options.padding) : 4;
    this.options.radius = _float(this.options.radius) || 50;
    this.options.thickness = _float(this.options.thickness) || 10;
    ...
    this.options.title = isDefined(this.options.title) ? this.options.title : function () {
      return "Donut Chart Graphic";
    };
    this.options.desc = isDefined(this.options.desc) ? this.options.desc : function (v) {
  ...
  donutty.prototype.insertFragments = function (values) {
    ...
    this.$wrapper.appendChild(this.$html); // because of a strange bug in browsers not updating
    ...
    this.$wrapper.innerHTML = this.$wrapper.innerHTML; // and because we just destroyed the DOM fragment and all
  ...
  donutty.prototype.getDashValues = function () {
    ...
    if (this.options.round && this.options.circle && percentageFilled < 100 && absoluteFilled < this.options.thickness) {
  ...
  donutty.prototype.animate = function (fill, full) {
    var _this = this; // ensure the transition property is applied before
...
})(document, window); // jquery constructor
